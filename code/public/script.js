...
/* Change 2: character counter element */
...
function bindSocket() {
  ...
  /* Change 1: handle joinError from server */
  socket.on('joinError', ({ message }) => {
         joinErr.textContent = message;
         joinBtn.disabled = false;
         joinLabel.textContent = 'Join Chat';
         socket.disconnect();
         socket = null;
  });
  ...
  /* Change 1: handle joinError during room switch */
  socket.on('joinError', ({ message }) => {
         addSys(message, 'error');
  });
}
...
function send() {
     ...
     /* Change 2: reset counter after send */
  updateCharCount(0);
     stopTyp();
}
...
/* Change 2: update the counter display */
...
function addMsg(m, own) {
  ...
  /* Change 3: format full time for tooltip (title) and short time for hover display */
  const fullTime  = new Date(m.time).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
     const shortTime = new Date(m.time).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' });
     ...
     row.innerHTML =
            ...
            '<div class="mbody">' +
              '<div class="mmeta">' +
                ...
                /* Change 3: time is now hidden, revealed on bubble hover via CSS */
                '<span class="mtime" title="' + fullTime + '">' + shortTime + '</span>' +
              '</div>' +
              /* Change 3: add title attribute to bubble for native tooltip fallback */
              '<div class="bubble" title="' + fullTime + '">' + esc(m.text) + '</div>' +
            '</div>';
